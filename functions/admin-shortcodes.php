...
function woo_shortcode_contactform ( $atts, $content = null ) {
		...
		if ( ( count( $_POST ) > 3 ) && isset( $_POST['submitted'] ) ) {
			...
			$error_responses = array(
									'contactName' => __( 'សូមមេត្តាបំពេញឈ្មោះរបស់អ្នក', 'woothemes' ),
									'contactEmail' => __( 'សូមបំពេញអាស័យដ្នានសារ និង​ច្បាស់ថាវាត្រឹមត្រូវ', 'woothemes' ),
									'contactMessage' => __( 'សូមបំពេញអត្ថន័យសាររបស់អ្នក', 'woothemes' )
									);
		...
		if( count( $error_messages ) ) {
			...
			$html .= do_shortcode( '[box type="alert"]' . __( 'សំណើររបស់អ្នកមានកំហុសមួយរឺច្រើនក្នុងការផ្ញើរ។', 'woothemes' ) . '[/box]' );
		...
		if ( $email == '' ) {} else {
			...
			$html .= '<p><label for="contactName">' . __( 'ឈ្មោះអ្នកផ្ញើរ​៖', 'woothemes' ) . '</label>' . "\n";
			...
			$html .= '<p><label for="contactEmail">' . __( 'អាស័យដ្នានសារ៖', 'woothemes' ) . '</label>' . "\n";
			...
			$html .= '<p class="textarea"><label for="contactMessage">' . __( 'អត្ថន័យសារ ៖', 'woothemes' ) . '</label>' . "\n";
			...
			$html .= '<p class="inline"><input type="checkbox" name="sendCopy" id="sendCopy" value="true"' . $sendCopy . ' /><label for="sendCopy">' . __( 'ផ្ញើរសារនេះទៅកាន់អាស័យដ្នាសាររបស់អ្នកដែរ', 'woothemes' ) . '</label></p>' . "\n";
			...
			$html .= '<p class="buttons"><input type="hidden" name="submitted" id="submitted" value="true" /><input class="submit button" type="submit" value="' . 'ផ្ញើរសារ' . '" /></p>';
...
function woo_shortcode_twitter_follow ( $atts, $content = null ) {
	...
	add_action( 'wp_footer', 'woo_shortcode_twitter_follow_js' );

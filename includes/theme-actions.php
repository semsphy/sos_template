...
function woo_custom_styling() {
	...
	if ( isset( $woo_options['woo_style_disable'] ) && $woo_options['woo_style_disable'] != 'true' ) :
		...
		if ( $slider_biz_font_excerpt )
			$output .= '.business #loopedSlider .content p { ' . woo_generate_font_css( $slider_biz_font_excerpt, 1.5 ) . ' }'. "\n";	
	...
	jQuery(window).load(function(){
		if ( jQuery( '#loopedSlider .slide' ).length > 1 ) {
		    jQuery( '#loopedSlider' ).slides({
		        ...
		        slidesLoaded: function () { jQuery( '#loopedSlider .slides' ).css( 'height', jQuery( '#loopedSlider .slide:first' ).height() ); }
		    });
...
if ( ! function_exists( 'woo_nav_subscribe' ) ) {
	function woo_nav_subscribe() {
		...
		<ul class="rss fr">
			...
			<?php } ?>
			<li class="sub-rss"><a href="<?php if ( $woo_options['woo_feed_url'] ) { echo $woo_options['woo_feed_url']; } else { echo get_bloginfo_rss( 'rss2_url' ); } ?>"><?php _e( 'អានតាមរយៈ RSS', 'woothemes' ); ?></a></li>
		</ul>
		<?php }
...
if ( ! function_exists( 'woo_post_more' ) ) {
	...
	?>
		<div class="post-more">   
			<?php
				echo str_replace(array("Read full story"), array("អានបន្ត"), $html) ;
			?>
		</div>                        	
	<?php
			}
...
if ( ! function_exists( 'woo_post_meta' ) ) { 
 function woo_post_meta() {
	...
	printf( '<div class="post-meta">%s</div>' . "\n", str_replace(array('by','By','on'),array('ដោយ៖','ដោយ៖','ថ្ងៃទី'),khmer_date(apply_filters( 'woo_filter_post_meta', $post_info ))));

       ...
       ?>
		...
		<?php echo $before_widget; ?>
 		<div id="tabs">
            ...
            <ul class="wooTabs">
                <?php if ( $order == "latest" && !$latest == "on") { ?><li class="latest"><a href="#tab-latest"><?php _e( 'អត្ថបទថ្មីៗ', 'woothemes' ); ?></a></li>
                <?php } elseif ( $order == "comments" && !$comments == "on") { ?><li class="comments"><a href="#tab-comm"><?php _e( 'មតិយោបល់', 'woothemes' ); ?></a></li>
                <?php } elseif ( $order == "tags" && !$tags == "on") { ?><li class="tags"><a href="#tab-tags"><?php _e( 'Tags', 'woothemes' ); ?></a></li>
                <?php } ?>
                <?php if (!$pop == "on") { ?><li class="popular"><a href="#tab-pop"><?php _e( 'និយមអាន', 'woothemes' ); ?></a></li><?php } ?>
                <?php if ($order <> "latest" && !$latest == "on") { ?><li class="latest"><a href="#tab-latest"><?php _e( 'អត្ថបទថ្មីៗ', 'woothemes' ); ?></a></li><?php } ?>
                <?php if ($order <> "comments" && !$comments == "on") { ?><li class="comments"><a href="#tab-comm"><?php _e( 'មតិយោបល់', 'woothemes' ); ?></a></li><?php } ?>
...
function KhmerDate(day,month,year){
	
	this.dayEn = day;
	this.monthEn = month;
	this.yearEn = year.replace(/^\s\s*/, '').replace(/\s\s*$/, '');
	...
	this.monthEns = ['jan','feb','mar','apr','may','jun','july','aug','sep','oct','nov','dec'];
	this.monthKhmers = ['មករា','គុម្ភះ','មិនា','មេសា','ឩ​​សភា','មិថុនា','កក្កដា','សីហា','កញ្ញា','តុលា','វិច្ឆិការ','ធ្នូ'];
	this.numberKhmers = ['០','១','២','៣','៤','៥','៦','៧','៨','៩'];
	this.dayNameKhmers = ['ច័ន្ទ','អង្គារ','ពុធ','ព្រហស្បតិ៍','សុក្រ','សៅរ៍','អាទិត្យ'];

	this.init();
}

KhmerDate.prototype.init = function(){
	this.loadYearKhmer();
	this.loadDayKhmer();
	this.loadMonthKhmer();
}

KhmerDate.prototype.getDateKhmer = function(){
	return this.dayKhmer + ' ' + this.monthKhmer + ' ' + this.yearKhmer;
}

KhmerDate.prototype.toString = function (){
	return this.dayKhmer + ' ' + this.monthKhmer + ' ' + this.yearKhmer;
}

KhmerDate.prototype.loadDayKhmer = function(){
	for(var i=0;i<this.dayEn.length;i++){
		var index = parseInt(this.dayEn.substr(i,1));
		this.dayKhmer+=this.numberKhmers[index];
	}	
}

KhmerDate.prototype.loadYearKhmer=function(){
	for(var i=0;i<this.yearEn.length;i++){
		var index = parseInt(this.yearEn.substr(i,1));
		this.yearKhmer+=this.numberKhmers[index];
	}
}

KhmerDate.prototype.loadMonthKhmer=function(){
	this.monthKhmer=this.monthKhmers[this.monthEns.indexOf(this.monthEn)];
}
jQuery(document).ready(function(){
	...
	jQuery('#tab-pop li span').each(function(ind,obj){
		var T_text = jQuery(obj).text().split(',');
		var year = T_text[1]
		var month_day = T_text[0].split(' ');
		var month = month_day[0].substr(0,3).toLowerCase();
		var day = month_day[1];

		var dateKhmer = new KhmerDate(day,month,year);
		jQuery(obj).text(dateKhmer.getDateKhmer());
	});

	jQuery('#tab-latest li span').each(function(ind,obj){
		var T_text = jQuery(obj).text().split(',');
		var year = T_text[1]
		var month_day = T_text[0].split(' ');
		var month = month_day[0].substr(0,3).toLowerCase();
...
if ( ! function_exists( 'woo_widget_tabs_comments' ) ) {
	function woo_widget_tabs_comments( $posts = 5, $size = 35 ) {
		global $wpdb;
		#SoS : to display the thumnail for commnent
		$size = 45;
